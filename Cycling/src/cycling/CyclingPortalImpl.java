...
public class CyclingPortalImpl implements Serializable{
  ...
  void removeRider(int riderId) throws IDNotRecognisedException {
    ...
    if (Rider.checkID(riderId, riders) == false) {
      ...
      // update races since they might have the rankings calculated already
      // we need to recalculate them
      for (Race race : races) {
        try {
          int [] array = this.getRidersGeneralClassificationRank(race.getObjectID());
        } catch (Exception e) {
  ...
  void deleteRiderResultsInStage(int stageId, int riderId)
    throws IDNotRecognisedException {
      ...
      //UPDATE THE adjustedElapsedTimesOfTheRace by using getRidersGeneralClassificationRank
      ////////////////////////////////////////////////////////
      for (Stage stage : stages) {
        if (stage.getStageId() == stageId) {
          try {
            this.getRidersGeneralClassificationRank(stage.getRaceId());
          } catch (Exception e) {
            // TODO: handle exception
          }
        }
  ...
  int[] getRidersRankInStage(int stageId) throws IDNotRecognisedException {
    ...
    // create an emptty int array to store thch
  ...
  /**
    ...
    // shoulf match getRidersGeneralClassificationRank
    // ie if 2,1,3. then the first point should be for 2, second for 1, and the last for 3.
    // orderedIds should have all the riders of the race 
